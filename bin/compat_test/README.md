...
当输入/输出包含非法或不可表示的字节序列时，可开启字节模式：

- `byteMode`: 布尔值，默认 `false`
  - `true` 时，本用例的 `args` / `tstdin` / `stdout` / `stderr` 都视为**十六进制字节串**  
  - `false` 时，按普通 UTF‑8 文本处理

**使用规则（简化版）**：`byteMode=true` 则整条用例走字节模式；否则走正常模式。
