...
#[test]
fn data_pipeline_internal_help_and_version_for_get_where_run_external() {
    for cmd in ["get", "where", "run-external"] {
        ...
        assert!(
            stdout.contains(&format!("syskits structured data pipeline {cmd} command")),
            "{expr} stdout: {stdout:?}"
        );
