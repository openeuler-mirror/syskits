    ...
    fn cp_from_matches(args_match: &ArgMatches) -> CopyResult<Self> {
        ...
        let cp_options = Self {
            ...
            dereference: if args_match.get_flag(opt_flags::DEREFERENCE) {
                ...
                || args_match.get_flag(opt_flags::CLI_SYMBOLIC_LINKS)
            {
                false
...
fn copy_source(
    ...
    let _source_str = source.as_os_str().to_string_lossy();
    ...
    if is_dir && target.exists() && target.is_dir() && !options.parents {
        ...
        if ends_with_dot {
            ...
            let is_empty = match fs::read_dir(source) {
                Ok(mut entries) => !entries.any(|e| match e.as_ref() {
                    Ok(entry) => {
...
fn cp_handle_existing_dest(
    ...
    // 提前剥离覆盖模式的核心策略，确保 -i 无法屏蔽 -f 的 unlink 行为
    let clobber_mode = match cp_opts.overwrite {
        CpOverwriteMode::Clobber(c) => c,
        CpOverwriteMode::Interactive(c) => c,
        CpOverwriteMode::NoClobber => CpClobberMode::Standard,
    };
    ...
    match clobber_mode {
        ...
        CpClobberMode::RemoveDestination => {
            let _ = fs::remove_file(dest_path);
        }
        CpClobberMode::Standard => {
