...
impl Base64Intent {
    fn from_call(call: &DataCall) -> Result<Self, CtDiagnosticError> {
        ...
        argv.push(OsString::from("base64"));
...
impl Base64Core {
    fn run_core(
        ...
        let semantic = ctengine::run_with_optional_pipeline_stdin(
            ...
            || {
                ct_base64::base64_native_semantic(intent.argv.iter().cloned())
                    .map_err(|err| CtDiagnosticError::simple(err.to_string()).with_code(err.code()))
            },
            |err| CtDiagnosticError::simple(format!("base64: {err}")),
        )?;
        Ok((
...
fn semantic_to_value(semantic: &ct_base64::Base64Semantic) -> CtValue {
    CtValue::List(semantic.rows.iter().map(row_to_value).collect())
}

fn row_to_value(row: &ct_base64::Base64SemanticRow) -> CtValue {
    CtValue::Record(vec![
...
impl DataCommand for CmdBase64 {
    fn signature(&self) -> DataSignature {
        DataSignature::new("base64", "structured base64 rows")
    ...
    fn run(
        ...
        let intent = Base64Intent::from_call(call)?;
        let (value, classic_text, classic_bytes, stderr_text, exit_code) =
            Base64Core::run_core(&intent, input)?;
...
#[cfg(test)]
mod tests {
    use super::{Base64Intent, display_columns, semantic_to_value};
    ...
    fn from_call_builds_argv_from_flags_and_positionals() {
        let mut call = DataCall::named("base64");
        ...
        let intent = Base64Intent::from_call(&call).expect("intent");
    ...
    fn semantic_to_value_renders_rows() {
        let value = semantic_to_value(&ct_base64::Base64Semantic {
            rows: vec![ct_base64::Base64SemanticRow {
                mode: "encode".into(),
                wrap: 76,
