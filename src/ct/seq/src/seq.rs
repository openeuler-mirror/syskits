...
use ctcore::ct_error::{CTResult, CTError, FromIo};
...
fn get_sequence_range(numbers: &[String]) -> CTResult<(PreciseNumber, PreciseNumber, PreciseNumber)> {
    let first = if numbers.len() > 1 {
        numbers[0].parse().map_err(|e| SeqError::ParseError(numbers[0].clone(), e))?
    } else {
        PreciseNumber::one()
    };

    let increment = if numbers.len() > 2 {
        let inc: PreciseNumber = numbers[1].parse().map_err(|e| SeqError::ParseError(numbers[1].clone(), e))?;
        if inc.is_zero() {
...
fn calculate_padding(first: &PreciseNumber, increment: &PreciseNumber, last: &PreciseNumber) -> usize {
    first.num_integral_digits
...
fn calculate_largest_decimal(first: &PreciseNumber, increment: &PreciseNumber) -> usize {
    first.num_fractional_digits.max(increment.num_fractional_digits)
}

fn parse_format_option(format_str: Option<&str>) -> CTResult<Option<Format<num_format::Float>>> {
    match format_str {
        Some(f) => Format::<num_format::Float>::parse(f).map(Some).map_err(|e| Box::new(e) as Box<dyn CTError>),
        None => Ok(None),
    }
}
...
fn print_seq(
    ...
    let mut writer: Box<dyn Write> = if let Some(buf) = buffer {
        Box::new(buf)
    } else {
        Box::new(stdout().lock())
    };
    ...
    while !done_printing(&value, &increment, &last) {
        ...
        match &format {
            ...
            None => write_value_float(&mut writer, &value, padding, largest_dec)?,
        }
