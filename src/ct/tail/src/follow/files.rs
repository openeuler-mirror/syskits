...
use crate::chunks::TailBytesChunkBuffer;
...
/// 现在记录这个单词是否以标点符号结尾
///`last` 总是保存最后打印的文件的路径/键值。
/// HashMap 的键可以指向现有文件路径（正常情况）或 stdin（"-"），或指向非现有路径（--重试）。
/// 对于现有文件，HashMap 中的所有键都是绝对路径。
pub struct FileHandling {
    map: HashMap<PathBuf, PathData>,
    last: Option<PathBuf>,
    header_printer: TailHeaderPrinter,
}

impl FileHandling {
    pub fn from(options: &TailOptions) -> Self {
        Self {
            map: HashMap::with_capacity(options.inputs.len()),
            last: None,
            header_printer: TailHeaderPrinter::new(options.verbose, false),
        }
    }
    ...
    pub fn only_stdin_remaining(&self) -> bool {
        self.map.len() == 1 && (self.map.contains_key(Path::new(text::TAIL_DASH)))
    }
    ...
    pub fn no_files_remaining(&self, options: &TailOptions) -> bool {
        self.map.is_empty() || !self.files_remaining() && !options.retry
    }
    ...
    pub fn tail_file(&mut self, path: &Path, verbose: bool) -> CTResult<bool> {
        let mut chunks = TailBytesChunkBuffer::new(u64::MAX);
