...
impl TestSymbol {
        ...
        match token {
            Some(s) => match s.to_str() {
                Some(t) => match t {
                    ...
                    "-eq" | "-ge" | "-gt" | "-le" | "-lt" | "-ne" => Self::Op(TestOperator::Int(s)),
                    "-ef" | "-nt" | "-ot" => Self::Op(TestOperator::File(s)),
                    "-n" | "-z" => Self::UnaryOp(TestUnaryOperator::StrlenOp(s)),
                    "-b" | "-c" | "-d" | "-e" | "-f" | "-g" | "-G" | "-h" | "-k" | "-L" | "-N"
                    | "-O" | "-p" | "-r" | "-s" | "-S" | "-t" | "-u" | "-w" | "-x" => {
                        Self::UnaryOp(TestUnaryOperator::FiletestOp(s))
                    }
                    _ => Self::Literal(s),
                },
                None => Self::Literal(s),
    ...
    fn into_literal(self) -> Self {
        Self::Literal(match self {
            ...
            | Self::UnaryOp(TestUnaryOperator::StrlenOp(s))
            | Self::UnaryOp(TestUnaryOperator::FiletestOp(s)) => s,
            Self::None => panic!(),
        })
    }
}
...
impl std::fmt::Display for TestSymbol {
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let s = match &self {
            ...
            | Self::Op(TestOperator::File(s))
            | Self::UnaryOp(TestUnaryOperator::StrlenOp(s))
            | Self::UnaryOp(TestUnaryOperator::FiletestOp(s)) => OsStr::new(s),
...
impl TestParser {
    ...
    fn peek_is_boolop(&mut self) -> bool {
        matches!(self.peek(), TestSymbol::BoolOp(_))
    }
    ...
    fn term(&mut self) -> ParseResult<()> {
        ...
        match symbol {
            TestSymbol::LParen => self.lparen()?,
            TestSymbol::Bang => self.bang()?,
            TestSymbol::UnaryOp(_) => self.uop(symbol),
            TestSymbol::None => self.stack.push(symbol),
    ...
    fn lparen(&mut self) -> ParseResult<()> {
        ...
        let peek3: Vec<TestSymbol> = self
            .tokens
            .clone()
            .take(3)
            .map(|token| TestSymbol::new(Some(token)))
            .collect();

        match peek3.as_slice() {
            // case 1: lparen is a literal when followed by nothing
            [] => {
                self.literal(TestSymbol::LParen.into_literal())?;
                Ok(())
            }
            ...
            //         or `( -f ( )`, for example
            [TestSymbol::UnaryOp(_), _, TestSymbol::Literal(s)] if s == ")" => {
                let symbol = self.next_token();
                self.uop(symbol);
                self.expect(")")?;
                Ok(())
            ...
            [TestSymbol::Op(_), TestSymbol::Literal(s)] | [TestSymbol::Op(_), TestSymbol::Literal(s), _]
                if s == ")" =>
            {
                self.literal(TestSymbol::LParen.into_literal())?;
                Ok(())
            ...
            [_, TestSymbol::Literal(s)] | [_, TestSymbol::Literal(s), _] if s == ")" => {
                let symbol = self.next_token();
                self.literal(symbol)?;
                self.expect(")")?;
                Ok(())
            ...
            [TestSymbol::Op(_), TestSymbol::Op(_), _] => {
                let symbol = self.next_token();
                self.literal(symbol)?;
                self.expect(")")?;
                Ok(())
            ...
            [TestSymbol::Op(_), _] | [TestSymbol::Op(_), _, _] => {
                self.literal(TestSymbol::LParen.into_literal())?;
                Ok(())
            }
        ...
        match self.peek() {
            ...
            _ => {
                ...
                match peek4.as_slice() {
                    ...
                    [TestSymbol::Literal(_), TestSymbol::BoolOp(_), TestSymbol::Literal(_)] => {
                        self.expr()?;
                        self.stack.push(TestSymbol::Bang);
                    }
                    _ => {
    ...
    fn maybe_boolop(&mut self) -> ParseResult<()> {
        if self.peek_is_boolop() {
            ...
            if let TestSymbol::None = self.peek() {
    ...
    /// first.
    fn boolop(&mut self, op: TestSymbol) -> ParseResult<()> {
        if op == TestSymbol::BoolOp(OsString::from("-a")) {
            self.term()?;
        } else {
            self.expr()?;
        }
    ...
    fn uop(&mut self, op: TestSymbol) {
        match self.next_token() {
            TestSymbol::None => self.stack.push(op.into_literal()),
            symbol => {
...
pub fn test_parse(args: Vec<OsString>) -> ParseResult<Vec<TestSymbol>> {
